...
async function loadShard(prefix) {
  ...
  if (!globalThis.DecompressionStream) throw new Error('Deze browser ondersteunt gecomprimeerde details niet');
  ...
  try {
    ...
    // Some static hosts transparently decode .gz responses before fetch sees them.
...
function updateExport() { const letters = selectedLetters(); $('#export-summary').textContent = letters.length ? letters.map(letter => letter.toUpperCase()).join(', ') : 'Kies minimaal één letter.'; $('#generate-pdf').disabled = !letters.length; }
